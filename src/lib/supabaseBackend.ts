...
async function resolveWorkspace(opts?: { name?: string; create?: boolean }): Promise<SessionInfo['workspace']> {
  ...
  let client = memberships?.[0]?.clients as unknown as
    | { id: string; name: string; contact_email: string | null; meta: Record<string, string> }
    | undefined
  if (!client && opts?.create !== false) {
    const displayName = opts?.name ?? 'הארגון שלי'
    const { data: created, error: cErr } = await supabase.rpc('create_client', {
      p_name: `ה-Workspace של ${displayName}`,
      p_slug: `ws-${Math.random().toString(36).slice(2, 8)}`,
    })
